"""
Default DETR model (v1.0) and criterion classes.
"""
print("DETR 1.0 network called")
import torch
...
class DETR(nn.Module):
	...
	def __init__(self, backbone, transformer, num_classes, num_queries, num_keys, aux_loss=False):
		...
		num_body_parts = (self.num_keys*3) # 32  (del_x1, del_y1, vis) * 32 + 2  # 17  (del_x1, del_y1, vis) * 17 + 2 
...
class SetCriterion(nn.Module):
	...
	def loss_keypoints(self, outputs, targets, indices, num_boxes):
		...
		#V_pred = src_keypoints[:, 64:]
	
		targets_keypoints = torch.cat([t['keypoints'][i] for t, (_, i) in zip(targets, indices)], dim=0)
		
		#C_gt = targets_keypoints[:, :2]keypointsbox
		Z_gt = targets_keypoints[:, :self.num_keys*2]
		
		#V_gt = targets_keypoints[:, 64:]

		# C_gt_expand = torch.repeat_interleave(C_gt.unsqueeze(1), 32, dim=1).view(-1,64)
		# A_gt = C_gt_expand + Z_gt
		# C_pred_expand = torch.repeat_interleave(C_pred.unsqueeze(1), 32, dim=1).view(-1,64)
