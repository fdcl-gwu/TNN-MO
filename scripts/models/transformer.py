...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        #self.plot_attention_heatmap(attn_weights_np)
    ...
    def plot_attention_heatmap(self, attn_weights, ax=None):
        """
        Plots a heatmap of the attention weights.
        
        Args:
            attn_weights (np.array): The attention weights to plot.
            ax (matplotlib.axes.Axes, optional): The axes on which to plot. If None, a new figure and axes are created.
        """
        if ax is None:
            fig, ax = plt.subplots()
        
        print("attn_output_weights np ", attn_weights)
        # Create the heatmap
        cax = ax.matshow(attn_weights[0], cmap='viridis')
        print("attn_output_weights np 2", attn_weights[0])
        # Add a colorbar
        fig.colorbar(cax)
        
        filename  = "/home/maneesh/Desktop/LAB2.0/TRPE_Multi_Object/attention.png"
        # Save the plot
